...
\begin{frame}{55. CCS-MPC Result: Dynamic $q$-axis Current Tracking ($i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Physical Inverter Tracking ($0 - 0.30\text{ s}$):} Shows actual measured winding current $i_{q,meas}$ tracking dynamic torque acceleration reference $i_{q,ref}$ (reaching $\sim 495\text{ A}$ transient peak).
        \item \textbf{High-Frequency Carrier Envelope:} The actual phase current tracks the reference with zero DC offset, centered within the theoretical $\Delta I_{Lq} \approx 70.8\text{ A}_{pk-pk}$ switching ripple band.
    \end{itemize}
\end{frame}
...
\begin{frame}{56. CCS-MPC Result: $q$-axis Current Tracking Error ($\Delta i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Symmetrical Zero-Centered Tracking Error:} Tracking error $\Delta i_q = i_{q,ref} - i_{q,meas}$ oscillates symmetrically around the zero baseline, with mean error $= \mathbf{+0.35\text{ A}}$ (deadbeat accuracy).
        \item \textbf{Ripple Justification:} Peak-to-peak ripple band matches the physical $\Delta I_{Lq} = \frac{V_{dc}}{4 f_{sw} L_q}$ limit, validating that the controller introduces zero low-frequency distortion.
    \end{itemize}
\end{frame}
...
\begin{frame}{57. CCS-MPC Result: Dynamic $d$-axis Current Tracking ($i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Demagnetizing MTPA Tracking ($0 - 0.30\text{ s}$):} Measured direct-axis current $i_{d,meas}$ reliably tracks the negative reluctance command $i_{d,ref}$ (down to $-273.2\text{ A}$).
        \item \textbf{Cross-Axis Decoupling:} Predictive feedforward decoupling completely isolates $d$-axis dynamics from rapid $q$-axis torque steps.
    \end{itemize}
\end{frame}
...
\begin{frame}{58. CCS-MPC Result: $d$-axis Current Tracking Error ($\Delta i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Zero Mean Tracking Error:} Mean steady-state error is strictly bounded at $\mathbf{-0.03\text{ A}}$, verifying exact deadbeat current synthesis.
        \item \textbf{Machine Saliency Verification:} Because $L_d \approx 0.5 L_q$ ($71.2\ \mu\text{H}$ vs $141.3\ \mu\text{H}$), the direct-axis ripple band is wider ($\Delta I_{Ld} \approx 140.5\text{ A}_{pk-pk}$), confirming physical motor saliency behavior.
    \end{itemize}
\end{frame}
