...
\begin{frame}{54a. Physical Current Ripple Justification: Theoretical $\Delta I_L$ Formulation}
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{3pt}
        ...
        \end{equation}
        \item \textbf{Axis-Specific Ripple Evaluation for BMW i3 IPMSM:}
        \begin{itemize}
            \footnotesize
            \item \textbf{$q$-axis Ripple ($L_q = 141.30\ \mu\text{H}$):}
            \begin{equation}
                \Delta I_{Lq} = \frac{400\text{ V}}{4 \times 10{,}000\text{ Hz} \times 141.30 \times 10^{-6}\text{ H}} = \mathbf{70.77\text{ A}_{pk-pk}} \quad (\pm 35.39\text{ A})
            \end{equation}
            \item \textbf{$d$-axis Ripple ($L_d = 71.20\ \mu\text{H}$):}
            \begin{equation}
                \Delta I_{Ld} = \frac{400\text{ V}}{4 \times 10{,}000\text{ Hz} \times 71.20 \times 10^{-6}\text{ H}} = \mathbf{140.45\text{ A}_{pk-pk}} \quad (\pm 70.22\text{ A})
            \end{equation}
        \end{itemize}
...
\begin{frame}{55. CCS-MPC Result: Dynamic $q$-axis Current Tracking ($i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{100\% Maximum Rated Torque Benchmark ($0 - 0.40\text{ s}$):} Steps across full motor capability up to rated peak torque: cruising ($230.0\text{ A}, 100\text{ Nm}$) $\rightarrow$ \textbf{Maximum Rated Torque ($460.0\text{ A}, 250.0\text{ Nm}$)} $\rightarrow$ dynamic hill-climb ($360.0\text{ A}, 180\text{ Nm}$).
        \item \textbf{Full Transient Settling Before Changing Inputs:} Each current step settles via deadbeat predictive action in $< 1\text{ ms}$ and maintains $> 120\text{ ms}$ to $140\text{ ms}$ of settled steady-state tracking before the subsequent step occurs.
        \item \textbf{High-Frequency Carrier Ripple at Max Torque:} Even at peak rated current ($460\text{ A}$), measured $i_q$ tracks with zero fundamental offset, enveloped within $\mathbf{35.83\text{ A}_{pk-pk}}$ ($\pm 17.92\text{ A}$, $6.33\%$ of $I_{max}$) ripple, strictly bounded below theoretical $\Delta I_{Lq,max} = 70.77\text{ A}_{pk-pk}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{56. CCS-MPC Result: $q$-axis Current Tracking Error ($\Delta i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Zero-Centered Symmetrical Error:} Across all plateaus including $460\text{ A}$ ($250\text{ Nm}$ peak), tracking error $\Delta i_q = i_{q,ref} - i_{q,meas}$ oscillates symmetrically around zero with mean fundamental error $< 1.5\text{ A}$ ($< 0.35\%$ of setpoint).
        \item \textbf{Physical Ripple Validation:} Settled peak-to-peak ripple measures $\mathbf{35.83\text{ A}_{pk-pk}}$ at peak torque, confirming carrier harmonics remain strictly within the theoretical maximum envelope $\Delta I_{Lq} = \frac{V_{dc}}{4 f_{sw} L_q} = 70.77\text{ A}_{pk-pk}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{57. CCS-MPC Result: Dynamic $d$-axis Current Tracking ($i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{MTPA Reluctance Torque Allocation for Max Torque ($0 - 0.40\text{ s}$):} Steps from $-75.0\text{ A} \rightarrow \mathbf{-240.0\text{ A}} \rightarrow -165.0\text{ A}$ to maximize reluctance torque alongside $q$-axis excitation, yielding $518.8\text{ A}$ peak stator vector.
        \item \textbf{Cross-Axis Decoupling \& Full Settling:} Direct axis settles completely in $< 1\text{ ms}$ with zero cross-coupling disturbance during $230\text{ A} \rightarrow 460\text{ A}$ torque steps, maintaining $> 120\text{ ms}$ settled tracking.
    \end{itemize}
\end{frame}
...
\begin{frame}{58. CCS-MPC Result: $d$-axis Current Tracking Error ($\Delta i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Zero Fundamental DC Bias:} Mean steady-state tracking error at max torque is bounded at $\approx 3.5\text{ A}$, verifying deadbeat predictive current synthesis even under heavy demagnetizing conditions.
        \item \textbf{Physical Saliency Verification:} Direct-axis settled ripple measures $\mathbf{64.55\text{ A}_{pk-pk}}$ ($\pm 32.28\text{ A}$, $11.41\%$ of $I_{max}$), tightly confirming machine electromagnetic saliency ($L_q \approx 2 L_d$, $141.3\ \mu\text{H}$ vs $71.2\ \mu\text{H}$) while remaining well below $\Delta I_{Ld,max} = 140.45\text{ A}_{pk-pk}$.
    \end{itemize}
\end{frame}
