...
\begin{frame}{51. Speed MPC: Multi-Regime Operational Allocation Table}
    \centering
    \small
    ...
    \begin{tabular}{l c c c}
        ...
        \rowcolor{blue!5} \textbf{MIL vs. SIL Discrepancy} & \textbf{5.68e-14 A} (Epsilon Match) & \textbf{1.14e-13 A} (Epsilon Match) & \textbf{0.00e+00 A} (Bit-Exact) \\
        \bottomrule
    \end{tabular}%
    }
    \vspace{3mm}
...
\begin{frame}{54a. Physical Current Ripple Justification: Theoretical $\Delta I_L$ Formulation}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{equation}
        \end{itemize}
        \item \textbf{Physical Interpretation of Tracking Error:}
        \begin{itemize}
            \footnotesize
            \item \textbf{High-Frequency Carrier Ripple:} The instantaneous measured currents oscillate strictly within the theoretical $\pm \frac{\Delta I_L}{2}$ envelope dictated by inverter PWM switching physics.
            \item \textbf{Deadbeat Fundamental Convergence:} CCS-MPC forces the \textbf{mean fundamental tracking error to zero ($< 0.5\text{ A}$)}, eliminating steady-state offset without artificial numerical filtering.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{69. CCS-MPC: Current & Voltage Tracking Performance Metrics}
    \centering
    \small
    \textbf{Dynamic Performance Metrics under Dynamic Torque Loading (Up to $250\text{ N}\cdot\text{m}$ Max Torque)}
    \vspace{1mm}
    \resizebox{0.95\textwidth}{!}{%
    ...
    \begin{tabular}{l c c c c}
        ...
        \midrule
        \rowcolor{blue!5} \textbf{Theoretical Ripple Bound ($\Delta I_L$)} & \textbf{140.45 A$_{pk-pk}$} & \textbf{70.77 A$_{pk-pk}$} & N/A & N/A \\
        \textbf{Measured Inverter Ripple} & \textbf{64.55 A$_{pk-pk}$} ($\pm 32.3\text{ A}$) & \textbf{35.83 A$_{pk-pk}$} ($\pm 17.9\text{ A}$) & $\pm 12.5\text{ V}$ band & $\pm 18.2\text{ V}$ band \\
        \rowcolor{blue!5} \textbf{Fundamental Steady-State Mean Error} & \textbf{$< 3.5\text{ A}$ ($< 1.5\%$)} & \textbf{$< 1.5\text{ A}$ ($< 0.35\%$)} & Carrier-Filtered & Carrier-Filtered \\
        \textbf{MIL vs. SIL Absolute Parity} & \textbf{0.00e+00 A} & \textbf{0.00e+00 A} & \textbf{0.00e+00 V} & \textbf{0.00e+00 V} \\
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Deadbeat Regulation:} Settles current step reference within 1 discrete sample period ($100\ \mu\text{s}$) with zero cross-axis coupling.
        \item \textbf{Saliency Confirmation:} Direct-axis ripple ($64.55\text{ A}_{pk-pk}$) vs quadrature-axis ripple ($35.83\text{ A}_{pk-pk}$) accurately tracks the machine inverse inductance ratio ($L_q / L_d = 1.98$).
    \end{itemize}
\end{frame}
