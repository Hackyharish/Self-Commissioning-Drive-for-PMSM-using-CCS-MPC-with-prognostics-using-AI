...
\begin{frame}{55. CCS-MPC Result: Dynamic $q$-axis Current Tracking ($i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Proper Automotive Current Profile ($0 - 0.40\text{ s}$):} Steps across realistic EV operating points: cruise acceleration ($180.0\text{ A}$) $\rightarrow$ heavy acceleration ($350.0\text{ A}$, $\sim 62\%$ of $I_{max}$) $\rightarrow$ nominal cruising ($220.0\text{ A}$).
        \item \textbf{Full Transient Settling Before Changing Inputs:} Each current step settles via deadbeat action in $< 1\text{ ms}$ and maintains $> 120\text{ ms}$ of unperturbed steady-state tracking before the subsequent step occurs.
        \item \textbf{High-Frequency Carrier Ripple:} Actual measured stator current $i_{q,meas}$ tracks the command with near-zero fundamental bias, enveloped within $\mathbf{36.7\text{ A}_{pk-pk}}$ ($\pm 18.4\text{ A}$) ripple, strictly bounded below theoretical $\Delta I_{Lq,max} = 70.77\text{ A}_{pk-pk}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{56. CCS-MPC Result: $q$-axis Current Tracking Error ($\Delta i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Zero-Centered Symmetrical Error:} Across all operating plateaus ($180\text{ A}$, $350\text{ A}$, $220\text{ A}$), tracking error $\Delta i_q = i_{q,ref} - i_{q,meas}$ oscillates symmetrically around zero with mean error $< 2.7\text{ A}$ ($< 0.8\%$ of setpoint).
        \item \textbf{Physical Ripple Validation:} Settled peak-to-peak ripple measures $\mathbf{36.7\text{ A}_{pk-pk}}$ ($6.49\%$ of $I_{max}$), remaining comfortably within the theoretical maximum envelope $\Delta I_{Lq} = \frac{V_{dc}}{4 f_{sw} L_q} = 70.77\text{ A}_{pk-pk}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{57. CCS-MPC Result: Dynamic $d$-axis Current Tracking ($i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Proper MTPA Reluctance Steps ($0 - 0.40\text{ s}$):} Steps from $-30.0\text{ A} \rightarrow -100.0\text{ A} \rightarrow -50.0\text{ A}$ to generate optimal reluctance torque aligned with $q$-axis demands.
        \item \textbf{Cross-Axis Decoupling \& Full Settling:} Direct axis settles completely with zero cross-coupling spikes during $q$-axis transitions, maintaining $> 120\text{ ms}$ of settled tracking per step.
    \end{itemize}
\end{frame}
...
\begin{frame}{58. CCS-MPC Result: $d$-axis Current Tracking Error ($\Delta i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Zero Fundamental DC Bias:} Mean steady-state tracking error is strictly bounded at $\approx 3.5\text{ A}$, proving deadbeat predictive current synthesis.
        \item \textbf{Physical Saliency Verification:} Direct-axis settled ripple measures $\mathbf{74.5\text{ A}_{pk-pk}}$ ($\pm 37.3\text{ A}$), which is precisely $\approx 2\times$ the $q$-axis ripple ($36.7\text{ A}_{pk-pk}$), confirming machine electromagnetic saliency ($L_q \approx 2 L_d$, $141.3\ \mu\text{H}$ vs $71.2\ \mu\text{H}$).
    \end{itemize}
\end{frame}
