...
\begin{frame}{54a. Physical Current Ripple Justification \& Symmetric SVPWM Architecture}
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{3pt}
        ...
        \end{itemize}
        \item \textbf{Symmetric Center-Aligned SVPWM \& Physical Ripple Reduction:}
        Replacing single-slope sawtooth modulation with dual-slope symmetric triangular carrier centers pulses around $T_{sw}/2$ via 7-segment sequence ($V_0 \to V_1 \to V_2 \to V_7 \to V_2 \to V_1 \to V_0$), reducing raw physical ripple by \textbf{87.8\%} ($362.7\text{ A} \to 44.1\text{ A}_{pk-pk}$) and eliminating the $1666.7\text{ Hz}$ subharmonic beat.
    \end{itemize}
\end{frame}
...
\begin{frame}{55. CCS-MPC Result: Dynamic $q$-axis Current Tracking ($i_q$)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Physical Ripple Bounded \& Clean Acquisition:} Dual-slope center-aligned SVPWM keeps raw winding ripple strictly within $\Delta I_{Lq,max} = 70.8\text{ A}$, while standard ADC anti-aliasing yields clean fundamental tracking with settled ripple $\le \pm 3.9\text{ A}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{56. CCS-MPC Result: $q$-axis Current Tracking Error ($\Delta i_q$)}
    ...
    \begin{itemize}
        ...
        \item \textbf{High Fundamental Tracking Accuracy:} Mean steady-state tracking error settles to $e_{ss} = +4.34\text{ A}$ (\textbf{99.1\% accuracy} at $467.07\text{ A}$ rated current), with settled steady-state ripple tightly bounded within the $\pm 3.9\text{ A}$ error envelope.
    \end{itemize}
\end{frame}
...
\begin{frame}{58. CCS-MPC Result: $d$-axis Current Tracking Error ($\Delta i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{High Fundamental Demagnetization Accuracy:} Mean steady-state direct-axis error is bounded at $e_{ss} = -7.03\text{ A}$ (\textbf{97.3\% accuracy} under $-254.25\text{ A}$ MTPA demagnetization bias).
        \item \textbf{Settled Ripple Verification:} Direct-axis settled ripple measures $\le \pm 10.0\text{ A}$ ($20.0\text{ A}_{pk-pk}$), demonstrating clean and robust direct-axis current regulation under deep reluctance torque generation.
    \end{itemize}
\end{frame}
