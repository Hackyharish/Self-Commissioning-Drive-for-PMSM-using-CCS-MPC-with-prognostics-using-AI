...
\begin{frame}{20. FOC vs CCS-MPC: Varying Load Comparison}
    \centering
    \footnotesize\textbf{Summary Table (Base Torque = -250 Nm, Speed = 800 rad/s, Sim Time = 0.6s)}
    \vspace{1mm}
...
\begin{frame}{21. Why We Need CCS-MPC (Load Analysis)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Superior Speed Regulation \& Lower Overshoot:} Across all operating loads, CCS-MPC maintains speed overshoot below \textbf{$0.17\%$} (801.3 rad/s max), whereas FOC exhibits up to \textbf{$0.97\%$} overshoot (807.8 rad/s peak).
...
\begin{frame}{38. AutoTuner Result: Speed Setpoint Output ($\omega_{ref,out}$)}
    \centering
...
\begin{frame}{48. Speed MPC Result: Speed Tracking Error ($e_\omega = \omega_{ref} - \omega_e$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Transient Error Dynamics ($0 - 2.5\text{ s}$):} Error peaks briefly ($\approx 54.5\text{ rad/s} \equiv 86.7\text{ RPM}$) during steep acceleration transitions ($4166\text{ rad/s}^2$).
    \end{itemize}
\end{frame}
...
\begin{frame}{72. Master Verification Summary: Complete MIL vs. SIL Equivalence}
    ...
    \begin{tabular}{l l c c c c}
        ...
        \rowcolor{blue!5} \textbf{AutoTuner} & Speed Setpoint Output ($\omega_{ref,out}$) & rad/s & \textbf{0.00e+00} & \textbf{0.00e+00} & \textbf{Bit-Exact Match} \\
        \midrule
        \textbf{Speed MPC} & Mechanical Speed Tracking ($\omega_e$) & rad/s & \textbf{0.00e+00} & \textbf{0.00e+00} & \textbf{Bit-Exact Match} \\
