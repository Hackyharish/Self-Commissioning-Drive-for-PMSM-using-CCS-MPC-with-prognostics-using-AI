...
\begin{frame}{47. Speed MPC Result: Speed Tracking Profile ($N$ [RPM])}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Closed-Loop Speed Step Response ($0 - 0.40\text{ s}$ Timeline in RPM):} Reference step command ($0 \rightarrow 318.3\text{ RPM}$ at $t=0.05\text{ s}$, corresponding to $200\text{ rad/s}$ electrical with $p = 6$ pole pairs).
        \item \textbf{Tracking Fidelity:} Measured rotor speed $N$ (solid blue) smoothly tracks reference $N_{ref}$ (dashed black) with high dynamic precision and zero overshoot.
    \end{itemize}
\end{frame}
...
\begin{frame}{48. Speed MPC Result: Speed Tracking Error ($e_{RPM} = N_{ref} - N_{meas}$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Transient Error Dynamics ($0 - 0.40\text{ s}$):} Error peaks briefly during the step transition ($318.3\text{ RPM}$) and settles cleanly to steady-state ($< 0.1\text{ RPM}$) within $50\text{ ms}$.
    \end{itemize}
\end{frame}
