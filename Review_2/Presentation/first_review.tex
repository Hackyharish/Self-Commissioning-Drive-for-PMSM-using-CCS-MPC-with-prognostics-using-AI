...
\begin{frame}{28a. Benchmark Motor Parameters: BMW i3 Traction IPMSM}
    ...
    \begin{itemize}
        ...
        \item \textbf{Ground-Truth Reference for AutoTuner:} Physical motor parameters ($R_s, L_d, L_q, \Psi_{PM}, J, B$) serve as the plant baseline to evaluate AutoTuner identification convergence and MIL/SIL equivalence.
        \item \textbf{Peak Mechanical Power ($P_{max} = 108.2\text{ kW}$):} Formulated via $P_{max} = T_{rated} \cdot \omega_{base} = 258.2\text{ N}\cdot\text{m} \times 418.9\text{ rad/s} = 108.15\text{ kW}$ ($104.7\text{ kW}$ at $250\text{ N}\cdot\text{m}$ test load), exactly matching the flat Region II constant-power profile in Slide 67.
        \item \textbf{Drive Optimization Constraints:} $I_{max} = 565.68\text{ A}$ and $V_{dc} = 400.0\text{ V}$ define current limit circles and dynamic voltage limit ellipses for MTPA, MTPV, and deadbeat CCS-MPC tracking.
    \end{itemize}
\end{frame}
...
\begin{frame}{29. Overall Self-Commissioning Cascaded Control Architecture}
    ...
    \begin{tikzpicture}[
        ...
        \node[ctrl] (pwm) at (4.9, 0) {SVPWM\\{\tiny (200 kHz Timer)}};
    ...
    \begin{itemize}
        \small
        \item \textbf{Decoupled Multi-Rate Architecture:} AutoTuner delivers identified motor constants prior to drive enable; Speed MPC ($1\text{ kHz}$) regulates mechanical dynamics; Inner CCS-MPC ($10\text{ kHz}$) synthesizes deadbeat voltages; SVPWM ($200\text{ kHz}$) switches the inverter with zero cross-talk.
    \end{itemize}
\end{frame}
