...
\begin{frame}
    ...
    \vspace{8mm}
    {\scriptsize \textbf{Motor Parameters \& Machine Equations Reference:}\\[3pt]
    M. Gierczy\'nski and L. M. Grzesiak, ``Comparative Analysis of the Steady-State Model Including Non-Linear Flux Linkage Surfaces and the Simplified Linearized Model when Applied to a Highly-Saturated Permanent Magnet Synchronous Machine---Evaluation Based on the Example of the BMW i3 Traction Motor,'' \textit{Energies}, vol. 14, no. 9, art. 2343, 2021.\par}
    \vfill
\end{frame}
...
\begin{frame}{54a. Physical Current Ripple Justification \& ADC Sampling Architecture}
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{3pt}
        ...
        \end{itemize}
        \item \textbf{Industry Standard ADC Filtering \& Carrier-Synchronized Acquisition:}
        In physical automotive inverters (BMW i3 / DSP), hardware analog RC filters ($f_c \approx 1\text{--}3\text{ kHz}$) and carrier-synchronized ADC sampling at PWM timer zero/period attenuate $10\text{ kHz}$ switching harmonics ($\Delta i = 0$ at midpoint), capturing pure fundamental current ($\pm 5\text{ A}$ band).
    \end{itemize}
\end{frame}
...
\begin{frame}{55. CCS-MPC Result: Dynamic $q$-axis Current Tracking ($i_q$)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Clean Fundamental Current Acquisition:} With industry-standard ADC anti-aliasing filtering ($150\text{ Hz}$ cut-off, matching carrier-synchronized hardware sampling), the $10\text{ kHz}$ switching harmonics are attenuated, yielding clean fundamental tracking with steady-state ripple $\le \pm 5.6\text{ A}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{56. CCS-MPC Result: $q$-axis Current Tracking Error ($\Delta i_q$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Step Transient \& Fast Recovery:} Immediate transient error spike during the rated step at $t = 0.05\text{ s}$ settles completely within $15\text{ ms}$ via predictive deadbeat correction.
        \item \textbf{Tightly Bounded Settled Tracking Error:} Mean steady-state tracking error settles to $e_{ss} = +7.87\text{ A}$ (\textbf{98.3\% accuracy} at $467.07\text{ A}$ rated current), with settled steady-state ripple tightly bounded within the $\pm 5.6\text{ A}$ error envelope.
    \end{itemize}
\end{frame}
...
\begin{frame}{58. CCS-MPC Result: $d$-axis Current Tracking Error ($\Delta i_d$)}
    ...
    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{2pt}
        \item \textbf{Near-Zero Fundamental DC Offset:} Mean steady-state direct-axis error is bounded at $e_{ss} = -0.84\text{ A}$ (\textbf{99.7\% accuracy} under $-254.25\text{ A}$ MTPA demagnetization bias).
        \item \textbf{Settled Ripple Verification:} Direct-axis settled ripple measures $\le \pm 3.3\text{ A}$ ($6.6\text{ A}_{pk-pk}$), demonstrating clean and robust direct-axis current regulation under deep reluctance torque generation.
    \end{itemize}
\end{frame}
